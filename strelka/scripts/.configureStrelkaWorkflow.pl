...
${STRELKA_ROOT}/strelka/etc/ for both ELAND and BWA alignments. (required)
...
BEGIN {
   ...
   $baseDir=$thisDir;
   $libDir=File::Spec->catdir($baseDir,'strelka','scripts');
}
use lib $libDir;
use Utils;
...
my $binDir = File::Spec->catdir($baseDir,'strelka','bin');
my $scriptDir = File::Spec->catdir($baseDir,'strelka','scripts');
my $samtoolsDir = File::Spec->catdir($baseDir,'redist','samtools');
checkDir($binDir,"strelka binaries");
checkDir($scriptDir,"strelka scripts");
...
my $callScript = File::Spec->catfile($scriptDir,$callScriptName);
my $filterScript = File::Spec->catfile($scriptDir,$filterScriptName);
my $finishScript = File::Spec->catfile($scriptDir,$finishScriptName);
my $countFasta = File::Spec->catfile($binDir,"countFastaBases");
